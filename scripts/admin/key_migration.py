...
3. Migrate all data (boards, tasks, prefs)
...
def migrate_key(old_key: str, new_key: str):
    ...
    print("[INFO] Step 1: Validating old key exists in KV...")
    ...
    if response.status_code == 404:
        print(f"[ERROR] Old key '{old_key}' has no data in KV")
        print(f"   Cannot migrate from a non-existent key")
        return 1
    print(f"[SUCCESS] Old key exists in KV")
    print()

    # Step 2: Validate new key exists in auth configuration
    print("[INFO] Step 2: Validating new key in authentication config...")
    key_exists, key_type = check_key_in_auth_config(new_key, env)

    if not key_exists:
        print(f"[WARNING] New key '{new_key}' not found in ADMIN_KEYS or FRIEND_KEYS")
        print()
        ...
        if response == 'y':
            ...
            if add_key_to_env(new_key, key_type, env_path, env_lines):
                print(f"[SUCCESS] Added to .env file")
                # Reload env
                env, env_lines, env_path = load_env()
            else:
                print("[ERROR] Failed to add key to .env")
                return 1
        else:
            print("[ERROR] Cannot migrate to a key that's not in auth config")
            return 1
    else:
        print(f"[SUCCESS] New key exists in {key_type.upper()}_KEYS")
    print()

    # Step 3: Migrate data
    print("[INFO] Step 3: Migrating KV data...")
    ...
    for prefix in old_prefixes:
        ...
        if not response.ok:
            print(f"[ERROR] Error listing keys: {response.text}")
            continue
        ...
        for key_obj in keys:
            ...
            if not response.ok:
                print(f"   [WARNING] Failed to read {old_full_key}")
                continue
    ...
    print("=" * 80)
    print("[INFO] Step 4: GitHub Secrets Update")
    print("=" * 80)
    ...
    if response == 'y':
        print("\n[INFO] Redeploying secrets to GitHub...")
        ...
        if result.returncode == 0:
            print("\n[SUCCESS] GitHub secrets updated successfully!")
        else:
            print("\n[WARNING] GitHub secrets update had issues. You may need to run manually:")
            print(f"   python scripts/administration.py github-secrets cloudflare")
    else:
        print("\n[WARNING] Skipping GitHub secrets update.")
