...
describe('Session KV Integration Tests', () => {
	...
	describe('Scenario 1: New Device Initialization', () => {
		it('should initialize new preferences object with defaults in KV', async () => {
			...
			const handshakeResponse = await app.request(
	...
	describe('Scenario 2: Existing Device Migration', () => {
		it('should fetch old preferences and move to new sessionId in KV', async () => {
			...
			const initialHandshake = await app.request(
				'/task/api/session/handshake',
				{
					method: 'POST',
					headers: adminHeaders,
			...
			const updatePrefs = await app.request(
			...
			const migrationHandshake = await app.request(
				'/task/api/session/handshake',
				{
					method: 'POST',
					headers: adminHeaders,
			...
			const newSessionInfo = await env.TASKS_KV.get<SessionInfo>(
	...
	describe('Scenario 3: Multi-Device with Different Themes', () => {
		it('should handle two devices with same authKey but different sessionIds and themes', async () => {
			...
			const device1Handshake = await app.request(
				'/task/api/session/handshake',
				{
					method: 'POST',
					headers: adminHeaders,
			...
			const device2Handshake = await app.request(
				'/task/api/session/handshake',
				{
					method: 'POST',
					headers: adminHeaders,
			...
			const device1PrefsInKV = await env.TASKS_KV.get<UserPreferences>(
				`prefs:${device1SessionId}`,
				'json'
			);
			const device2PrefsInKV = await env.TASKS_KV.get<UserPreferences>(
				`prefs:${device2SessionId}`,
				'json'
			);
			...
			const loadDevice2 = await app.request(
				'/task/api/preferences',
				{
					method: 'GET',
					headers: {
			...
			const loadDevice1 = await app.request(
				'/task/api/preferences',
				{
					method: 'GET',
					headers: {
			...
			const device1UpdatedInKV = await env.TASKS_KV.get<UserPreferences>(
				`prefs:${device1SessionId}`,
				'json'
			);
			...
			const device2StillInKV = await env.TASKS_KV.get<UserPreferences>(
				`prefs:${device2SessionId}`,
				'json'
			);
			...
			const device1Info = await env.TASKS_KV.get<SessionInfo>(
				`session-info:${device1SessionId}`,
				'json'
			);
			const device2Info = await env.TASKS_KV.get<SessionInfo>(
				`session-info:${device2SessionId}`,
				'json'
			);
