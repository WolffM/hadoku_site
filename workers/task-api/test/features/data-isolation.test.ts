...
describe('Data Isolation Tests', () => {
	...
	describe('Board Isolation', () => {
		it('should not share boards between admin and friend', async () => {
			...
			const adminCreateRes = await app.request(
				'/task/api/boards',
				{
					method: 'POST',
			...
			const friendCreateRes = await app.request(
				'/task/api/boards',
				{
					method: 'POST',
			...
			const friendListRes = await app.request(
				'/task/api/boards',
				{
					headers: friendHeaders,
				},
		...
		it('should isolate boards from public users', async () => {
			...
			const publicListRes = await app.request(
				'/task/api/boards',
				{
	...
	describe('Task Isolation', () => {
		it('should not share tasks between users', async () => {
			...
			const adminData = await adminListRes.json<{
				boards: { id: string; tasks: { id: string }[] }[];
			}>();
			...
			const friendData = await friendListRes.json<{
				boards: { id: string; tasks: { id: string }[] }[];
			}>();
			...
			expect(friendBoard).toBeDefined();
