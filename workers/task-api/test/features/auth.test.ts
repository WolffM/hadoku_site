...
describe('Authentication Tests', () => {
	...
	it('should accept key from query parameter', async () => {
		...
		const response = await app.request(
			'/task/api/boards?key=admin-key-123',
			{
				method: 'GET',
			},
			env
	...
	it('should allow Erin to create a task "Hi erin!" and verify it exists', async () => {
		...
		const getBoardsResponse = await app.request(
			'/task/api/boards',
			{
				method: 'GET',
				headers: erinHeaders,
		...
		const boardsData = (await getBoardsResponse.json()) as BoardsData;

		// Find Erin's board
		const erinBoard = boardsData.boards.find((b) => b.id === boardId);
		expect(erinBoard).toBeDefined();
		expect(erinBoard?.name).toBe("Erin's Board");

		// Find the task "Hi erin!"
		const hiErinTask = erinBoard?.tasks.find((t) => t.id === taskId);
