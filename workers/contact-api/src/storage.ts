 ...
 * KV key format: template:email:{name}:{language}
 */
export async function getEmailTemplate(
	...
	const kvKey = `template:email:${name}:${language}`;
	...
	// Cache in KV for next time (expire after 1 hour)
	if (template) {
		await kv.put(kvKey, JSON.stringify(template), { expirationTtl: 3600 });
	}
 ...
 * KV key format: template:chatbot:{name}:{language}
 */
export async function getChatbotPrompt(
	...
	// Try KV cache first
	const kvKey = `template:chatbot:${name}:${language}`;
	const cached = await kv.get(kvKey, 'json');
	if (cached) {
		return cached as ChatbotPrompt;
	}
	...
	// Cache in KV (expire after 1 hour)
	if (prompt) {
		await kv.put(kvKey, JSON.stringify(prompt), { expirationTtl: 3600 });
	}
...
export async function listEmailTemplates(
	...
	const { status, language, limit = 100, offset = 0 } = filters || {};
...
export async function upsertEmailTemplate(
	...
	await kv.put(kvKey, JSON.stringify(saved), { expirationTtl: 3600 });
